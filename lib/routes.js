        ...
        async.auto({
            ...
            renameObject: ['uploadInfo', function (obj, cb) {
                ...
                if (obj.uploadInfo.ETag !== req.backend.hash) {
                    log.info('Filename md5 hash mismatch, name:',
                        req.backend.hash,
                        ' hash:',
                        obj.uploadInfo.ETag,
                    ...
                    storage.deleteObject(
                        req.params.hashname + '.tmp', function (err) {
                            if (err) return cb(err);
                            return cb('hash mismatch', null);
                        }
                    );
                } else {
                    storage.renameObject(
                        req.params.hashname + '.tmp',
                        req.params.hashname,
                        obj.uploadInfo.contentType,
                        function (err) {
                            if (err) return cb(err);
                        });
            ...
            if (err) {
                if (err != 'hash mismatch') {
                    ...
                    return res.send(new errors.BadRequestError);
                }
            }
            return res.json(ok_json);
        });
